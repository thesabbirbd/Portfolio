...
import { PROFILE_DATA, CapabilityGroup } from "@/data/profile";
...
export function SkillsSection() {
  ...
  const filterMap: Record<string, string[]> = {
    all: ["backend", "devops", "networking", "ai", "hardware", "multimedia"],
    engineering: ["backend", "devops", "ai"],
    systems: ["devops", "networking", "hardware"],
    applied: ["ai", "hardware", "multimedia"],
  };

  const filteredCapabilities = PROFILE_DATA.capabilities.filter((c) =>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="space-y-3">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-mono font-semibold tracking-wider text-cyan-600 dark:text-cyan-400 bg-cyan-500/10 border border-cyan-500/20">
            ...
            <span>02 // CAPABILITY ECOSYSTEM</span>
          </div>
              ...
              >
                ...
                <div className="pt-4 border-t border-slate-200/50 dark:border-white/5 flex items-center justify-between text-[11px] font-mono text-slate-500 dark:text-slate-400">
                  <span className="flex items-center gap-1">
                    <Check className="w-3 h-3 text-emerald-500" /> Verified Domain
                  </span>
                  <span className="group-hover:translate-x-1 transition-transform text-blue-500 dark:text-cyan-400">
                    Active →
                  </span>
                </div>
              </GlassCard>
