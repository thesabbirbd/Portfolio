...
export function ThemeSwitcher({ className }: { className?: string }) {
  ...
  return (
    <motion.button
      ...
      whileTap={{ scale: 0.94 }}
      onClick={cycleTheme}
      className={`relative p-2 rounded-full glass-panel hover:border-cyan-400/40 transition-colors text-slate-700 dark:text-slate-200 flex items-center justify-center ${className || ""}`}
    ...
    >
      ...
      {theme === "dark" && <Moon className="w-4 h-4 text-cyan-400 transition-transform duration-300" />}
